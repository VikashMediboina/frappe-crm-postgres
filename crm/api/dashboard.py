...
# Database-agnostic SQL helper functions
def _get_date_add_sql(date_col, interval_days=1):
	"""Get database-agnostic SQL for adding days to a date."""
	if frappe.db.db_type == "postgres":
		# Cast parameter placeholders to date, column names don't need casting
		if date_col.startswith("%(") and date_col.endswith(")s"):
			return f"CAST({date_col} AS date) + INTERVAL '{interval_days} day'"
		else:
			return f"{date_col} + INTERVAL '{interval_days} day'"
	else:
		return f"DATE_ADD({date_col}, INTERVAL {interval_days} DAY)"


def _get_date_format_sql(date_col, format_str):
	"""Get database-agnostic SQL for formatting dates."""
	if frappe.db.db_type == "postgres":
		...
		return f"DATE_FORMAT({date_col}, '{format_str}')"


def _get_timestampdiff_sql(unit, date1, date2):
	"""Get database-agnostic SQL for calculating difference between dates."""
	if frappe.db.db_type == "postgres":
		# PostgreSQL uses EXTRACT or direct date subtraction
		if unit == "DAY":
			return f"EXTRACT(DAY FROM ({date2} - {date1}))"
		else:
			return f"EXTRACT({unit} FROM ({date2} - {date1}))"
	else:
		return f"TIMESTAMPDIFF({unit}, {date1}, {date2})"


def _get_date_sub_sql(date_col, interval_value, interval_unit):
	"""Get database-agnostic SQL for subtracting from a date."""
	if frappe.db.db_type == "postgres":
		# Convert MySQL interval units to PostgreSQL
		pg_unit = interval_unit.lower()
		if pg_unit == "MONTH":
			pg_unit = "months"
		# CURRENT_DATE is already a date type, parameter placeholders need casting
		if date_col == "CURRENT_DATE":
			return f"{date_col} - INTERVAL '{interval_value} {pg_unit}'"
		elif date_col.startswith("%(") and date_col.endswith(")s"):
			return f"CAST({date_col} AS date) - INTERVAL '{interval_value} {pg_unit}'"
		else:
			return f"{date_col} - INTERVAL '{interval_value} {pg_unit}'"
	else:
		return f"DATE_SUB({date_col}, INTERVAL {interval_value} {interval_unit})"


def _get_current_date_sql():
	"""Get database-agnostic SQL for current date."""
	if frappe.db.db_type == "postgres":
		return "CURRENT_DATE"
	else:
		return "CURDATE()"
...
def _get_group_concat_sql(column, separator=","):
	"""Get database-agnostic SQL for concatenating grouped values."""
	if frappe.db.db_type == "postgres":
...
def get_total_leads(from_date, to_date, user=""):
	...
	date_add_expr = _get_date_add_sql("%(to_date)s", 1)
	result = frappe.db.sql(
		f"""
		SELECT
            COUNT(CASE
                WHEN creation >= %(from_date)s AND creation < {date_add_expr}
                {conds}
                THEN name
                ELSE NULL
...
def get_ongoing_deals(from_date, to_date, user=""):
	...
	result = frappe.db.sql(
		...
		SELECT
			COUNT(CASE
				WHEN d.creation >= %(from_date)s AND d.creation < {date_add_expr}
...
def get_won_deals(from_date, to_date, user=""):
	...
	result = frappe.db.sql(
		...
		SELECT
			COUNT(CASE
				WHEN d.closed_date >= %(from_date)s AND d.closed_date < {date_add_expr}
					AND s.type = 'Won'
					{conds}
				THEN d.name
				ELSE NULL
...
def get_average_time_to_close_a_lead(from_date, to_date, user=""):
	...
	date_add_expr = _get_date_add_sql("%(to_date)s", 1)
	timestampdiff_expr = _get_timestampdiff_sql("DAY", "COALESCE(l.creation, d.creation)", "d.closed_date")
	result = frappe.db.sql(
		f"""
		SELECT
			AVG(CASE WHEN d.closed_date >= %(from_date)s AND d.closed_date < {date_add_expr}
				THEN {timestampdiff_expr} END) as current_avg_lead,
			AVG(CASE WHEN d.closed_date >= %(prev_from_date)s AND d.closed_date < %(prev_to_date)s
				THEN {timestampdiff_expr} END) as prev_avg_lead
		FROM `tabCRM Deal` AS d
		JOIN `tabCRM Deal Status` s ON d.status = s.name
...
def get_average_time_to_close_a_deal(from_date, to_date, user=""):
	...
	date_add_expr = _get_date_add_sql("%(to_date)s", 1)
	timestampdiff_expr = _get_timestampdiff_sql("DAY", "d.creation", "d.closed_date")
	result = frappe.db.sql(
		f"""
		SELECT
			AVG(CASE WHEN d.closed_date >= %(from_date)s AND d.closed_date < {date_add_expr}
				THEN {timestampdiff_expr} END) as current_avg_deal,
			AVG(CASE WHEN d.closed_date >= %(prev_from_date)s AND d.closed_date < %(prev_to_date)s
				THEN {timestampdiff_expr} END) as prev_avg_deal
		FROM `tabCRM Deal` AS d
		JOIN `tabCRM Deal Status` s ON d.status = s.name
		LEFT JOIN `tabCRM Lead` l ON d.lead = l.name
		WHERE d.closed_date IS NOT NULL AND s.type = 'Won'
...
def get_sales_trend(from_date="", to_date="", user=""):
	...
	date_format_expr = _get_date_format_sql("date", "%Y-%m-%d")
	result = frappe.db.sql(
		f"""
		SELECT
			{date_format_expr} AS date,
			SUM(leads) AS leads,
			SUM(deals) AS deals,
...
def get_forecasted_revenue(from_date="", to_date="", user=""):
	...
	date_format_expr = _get_date_format_sql("d.expected_closure_date", "%Y-%m")
	date_sub_expr = _get_date_sub_sql(_get_current_date_sql(), 12, "MONTH")
	result = frappe.db.sql(
		f"""
		SELECT
			{date_format_expr}                        AS month,
			SUM(
				CASE
					WHEN s.type = 'Lost' THEN d.expected_deal_value * COALESCE(d.exchange_rate, 1)
					ELSE d.expected_deal_value * COALESCE(d.probability, 0) / 100 * COALESCE(d.exchange_rate, 1)  -- forecasted
				END
			)                                                       AS forecasted,
			SUM(
				CASE
					WHEN s.type = 'Won' THEN d.deal_value * COALESCE(d.exchange_rate, 1)            -- actual
					ELSE 0
				END
			)                                                       AS actual
		FROM `tabCRM Deal` AS d
		JOIN `tabCRM Deal Status` s ON d.status = s.name
		WHERE d.expected_closure_date >= {date_sub_expr}
		{deal_conds}
		GROUP BY {date_format_expr}
		ORDER BY month
		""",
		params,
		as_dict=True,
...
def get_leads_by_source(from_date="", to_date="", user=""):
	...
	result = frappe.db.sql(
		...
		WHERE DATE(creation) BETWEEN %(from)s AND %(to)s
		{lead_conds}
		GROUP BY COALESCE(source, 'Empty')
		ORDER BY count DESC
		""",
		params,
		as_dict=True,
...
def get_deals_by_territory(from_date="", to_date="", user=""):
	...
	result = frappe.db.sql(
		f"""
		SELECT
			COALESCE(d.territory, 'Empty') AS territory,
			COUNT(*) AS deals,
			SUM(COALESCE(d.deal_value, 0) * COALESCE(d.exchange_rate, 1)) AS value
		FROM `tabCRM Deal` AS d
		WHERE DATE(d.creation) BETWEEN %(from)s AND %(to)s
		{deal_conds}
		GROUP BY COALESCE(d.territory, 'Empty')
...
def get_deals_by_salesperson(from_date="", to_date="", user=""):
	...
	result = frappe.db.sql(
		f"""
		SELECT
			COALESCE(u.full_name, d.deal_owner) AS salesperson,
			COUNT(*)                           AS deals,
			SUM(COALESCE(d.deal_value, 0) * COALESCE(d.exchange_rate, 1)) AS value
		FROM `tabCRM Deal` AS d
		...
		WHERE DATE(d.creation) BETWEEN %(from)s AND %(to)s
		{deal_conds}
		GROUP BY d.deal_owner, u.full_name
		ORDER BY deals DESC, value DESC
		""",
		params,
		as_dict=True,
